...
import { formatDateTimeLabel, getTaskStatusLabel } from "@/lib/utils";
...
export default async function TeamTasksPage({ params }: TeamTasksPageProps) {
  ...
  const { supabase, membership, user } = await requireTeamAccess(teamId, `/teams/${teamId}/tasks`);
  ...
  return (
    <div className="page-stack">
      <Card className="p-8">
        ...
        <p className="mt-3 text-muted-foreground">Checklisten, Zuständigkeiten und Orga-Punkte für Trainings- und Spieltage.</p>
        <div className="mt-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <StatsCard title="Offen" value={String(openTasks.length)} description="aktuelle Teamaufgaben" />
        <StatsCard title="Erledigt" value={String(doneTasks.length)} description="abgeschlossene Punkte" />
        <StatsCard title="Mit Bezug" value={String(tasks.filter((task) => task.event_id).length)} description="an Termine gekoppelt" />
      </section>
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Badge variant={task.status === "done" ? "success" : "muted"}>{getTaskStatusLabel(task.status)}</Badge>
                  </div>
                  ...
                  <div className="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                    ...
                    <p>Fällig: {task.due_at ? formatDateTimeLabel(task.due_at) : "ohne Frist"}</p>
                  ...
                  {canManage || task.assigned_to === user.id ? (
                    <form action={updateTaskStatusAction.bind(null, team.id, task.id)}>
                      ...
                      <SubmitButton variant="secondary" pendingLabel="Speichert...">
                        {task.status === "done" ? "Wieder öffnen" : "Als erledigt markieren"}
                      </SubmitButton>
                    </form>
                  ) : null}
                  {canManage ? (
                    <form action={deleteTaskAction.bind(null, team.id, task.id)}>
                      <ConfirmSubmit
                        type="submit"
                        variant="ghost"
                        confirmMessage="Aufgabe wirklich löschen?"
                      >
                        Löschen
                      </ConfirmSubmit>
                    </form>
