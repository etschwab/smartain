...
import { getTeamById, getTeamFeatureSupport } from "@/lib/data";
...
export default async function NewEventPage({ params }: NewEventPageProps) {
  ...
  const [team, featureSupport] = await Promise.all([getTeamById(supabase, teamId), getTeamFeatureSupport(supabase)]);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <Card className="p-8">
        ...
        <p className="mt-3 text-muted-foreground">Erstelle Training, Spiel, Besprechung oder Event fuer dein Team.</p>
        <div className="mt-6">
      ...
      <Card className="p-8">
        {featureSupport.events ? (
          ...
          <div className="space-y-3">
            <h2 className="text-2xl font-semibold">Terminmodul wird vorbereitet</h2>
            <p className="text-sm text-muted-foreground">
              Die Teamseite laeuft bereits, aber fuer neue Termine fehlt in Supabase noch die passende `events`-Tabelle.
            </p>
          </div>
        )}
      </Card>
